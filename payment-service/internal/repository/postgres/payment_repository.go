...
func (r *PaymentRepository) Create(ctx context.Context, payment *domain.Payment) error {
	const q = `
	INSERT INTO payments (id, order_id, transaction_id, amount, status, email)
	VALUES ($1, $2, $3, $4, $5, $6)`

	_, err := r.db.ExecContext(ctx, q,
		payment.ID, payment.OrderID, payment.TransactionID,
		payment.Amount, payment.Status, payment.Email,
	)
	if err != nil {
...
func (r *PaymentRepository) GetByOrderID(ctx context.Context, orderID string) (*domain.Payment, error) {
	const q = `
	SELECT id, order_id, transaction_id, amount, status, email
	FROM payments WHERE order_id = $1`

	var p domain.Payment
	err := r.db.QueryRowContext(ctx, q, orderID).Scan(
		&p.ID, &p.OrderID, &p.TransactionID, &p.Amount, &p.Status, &p.Email,
	)
...
func (r *PaymentRepository) FindByAmountRange(ctx context.Context, minAmount, maxAmount int64) ([]*domain.Payment, error) {
	...
	if minAmount > 0 && maxAmount > 0 {
		q = `SELECT id, order_id, transaction_id, amount, status, email
		...
		q = `SELECT id, order_id, transaction_id, amount, status, email FROM payments`
	}
	...
	for rows.Next() {
		var p domain.Payment
		if err := rows.Scan(&p.ID, &p.OrderID, &p.TransactionID, &p.Amount, &p.Status, &p.Email); err != nil {
			return nil, fmt.Errorf("PaymentRepository.FindByAmountRange: %w", err)
		}
